...
# replace na values with a predefined value (default=0)
na.replace <- function(x, value=0) { x[is.na(x)] <- value; x }
...
avg.or.first <- function(x, avg = mean) {
  # x = c(1, 2, 3)
  if (is.numeric(x)) y <- avg(x, na.rm=T)
  else y <- na.omit(x)[1]
  ...
  if (var %in% metadata$Var) { # known 
    ...
    for (col in names(meta_temp)) if (col %!in% c("New.levels")) {
      ...
      if (replace.exising.cells == T)
        ...
        # CONVERT EXISTING DATA TYPES TO NEW DATA TYPES !!!!
        # print ( c(col, typeof(metadata[[row, col]]), typeof(meta_temp[[1, col]]) ) )
        if ( typeof(metadata[[row, col]]) == "logical" ) {
          if ( typeof(meta_temp[[1, col]]) == "character" )
            metadata[[col]] <- as.character(metadata[[col]])
          if ( typeof(meta_temp[[1, col]]) %in% c("numeric", "double" ))
            metadata[[col]] <- as.numeric(metadata[[col]])
        }
        if ( typeof(metadata[[row, col]]) == "double" ) {
          if ( typeof(meta_temp[[1, col]]) == "character" )
            metadata[[col]] <- as.character(metadata[[col]])
        }
        metadata[[row, col]] <- meta_temp[[1, col]]
...
# check var type
is.binary <- function(x) return(is.factor(x) & length(levels(factor(x) )) == 2) # exactly 2 levels (actual data)

is.categ <- function(x, min.levels=3, max.levels=NA) { 
  nlevels <- length( levels( factor(x) ) )
  if (is.na(min.levels)) min.levels = 0 # allow factos with no levels
  if (is.na(max.levels)) max.levels = nlevels
  
  return(is.factor(x) & (nlevels <= max.levels) & (nlevels >= min.levels))
}
...
as.factor_metadata4 <- function(x, metadata, debug=T) {
  print(1/0)
  print("weryuio")
  return(11)
  label <- attr(x, "label")
  if (debug==T) print(label)
...
make_summary_table_cont <- function(db, continuous, vars=NULL, metadata=NULL, trim_to_metadata = T, gsd=F, gm_correction=0.001, r.method="spearman") {
  ...
  for (v in vars) {
    if (is.factor(db[[v]])) {
      t <- mySSby0(db, numerica=continuous, categ=v, gm_correction=gm_correction)
      tables[[v]] <- list(tab = t, test = attr(t, "test"), text = attr(t, "test_text"))
    } else {
      r = cor.test(db[[continuous]], db[[v]], method = r.method)
      n = nrow(db %>% select(c(continuous, v)) %>% na.omit)
      t <- data.frame(`Grup` = "(total)", #metadata.var_to_labels(v, metadata),
                      `n` = n, `%` = n/nrow(db),
                      `mean` = mean( db[[v]][ !is.na(db[[continuous]]) ], na.rm=T ),
                      `median` = median( db[[v]][ !is.na(db[[continuous]]) ], na.rm=T ),
                      `min` = min( db[[v]][ !is.na(db[[continuous]]) ], na.rm=T ),
                      `max` = max( db[[v]][ !is.na(db[[continuous]]) ], na.rm=T ),
                      `sd` = sd( db[[v]][ !is.na(db[[continuous]]) ], na.rm=T ),
                      `GM` = DescTools::Gmean( db[[v]][ !is.na(db[[continuous]]) ] + gm_correction, na.rm=T ),
                      `GSD` = DescTools::Gsd( db[[v]][ !is.na(db[[continuous]]) ] + gm_correction, na.rm=T )
                      ) %>% set_colnames(c("Grup", "n", "%", "mean", "median", "min", "max", "sd", "GM", "GSD"))
      
      attr(t, "test") <- r
      attr(t, "test_text") <- "(" %+% r$method %+% ": R=" %+% 
        scales::number(r$estimate, 0.001) %+% ", " %+%
        scales::pvalue(r$p.value, add_p = T) %+% ")"
      
      tables[[v]] <- list(`tab` = t, `test` = r, `text` = attr(t, "test_text"))
    }
  }
  
  
  res0 <- mySSby0(db, numerica=continuous, categ=NA, gm_correction=gm_correction)
  res0$Grup = "(total)"
  ...
  res1 <- res0 %>% mutate(
    ...
    `Mean ±SD` = paste0(scales::number(`mean`, 0.001), " ±", scales::number(`sd`, 0.01)),
    `Geometric mean, SD` = paste0(scales::number(`GM`, 0.01), ", ", scales::number(`GSD`, 0.01)),
    `Med (min:max)` = paste0(scales::number(`median`, 0.01), " (", scales::number(`min`, 0.01), ":", scales::number(`max`, 0.1), ")")
  )# %>%
  if(gsd==T) res1 %<>%
    select(`Subset` = `Grup`, `N`, `Mean ±SD`, `Med (min:max)`, `Geometric mean, SD`, `Grup`=`var`)
  else res1 %<>%
    select(`Subset` = `Grup`, `N`, `Mean ±SD`, `Med (min:max)`, `Grup`=`var`)
